...
import OtherProductData from '../components/OtherProductData'
...
const ProductCart = () => {
    const [cartData, setCartData] = useState(OtherProductData);
                    ...
                    <div className="max-w-sm rounded overflow-hidden shadow-lg" key={item.id}>
                        <div className='p-4'>
                            <NavLink to={`/productdetail/${item.id}`} onClick={()=>redirectingpage(item)}>
                                <img className="w-full h-[200px] object-contain border rounded-lg p-4" src={item.imgdata} alt="Sunset in the mountains" />
                            </NavLink>
                        </div>
                        <div className="px-8 py-4">
                            <div className="font-bold text-xl mb-2 truncate">{item.dish}</div>
